---
id: version-1.0.0-sudocu
title: Desplegar Sudocu 1.0
sidebar_label: Desplegar Sudocu 1.0
original_id: sudocu
---

Desde la raíz del  [repositorio](repo-config.md) navegue a la carpeta donde se encuentra la configuración de Sudocu 1.0
...
1. Inicializar la base de datos. Cambiar los valores de conexión.
   ...
   docker run --rm \
     ...
     --env SUDOCU_DB_PASSWORD=postgres \
     ungs/sudocu-db-instalador:1.0.1
   ```
...
1. Ingrese a Araí-Usuarios (user `admin` y password seteado [anteriormente](arai.md#creación-de-usuario-admin))
